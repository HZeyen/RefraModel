...
class Inversion:
    ...
    def run_inversion(self, params, scheme, bodies, points, lines,
                      forward_model, body_regularization=None):
...
# Set maximum iterations
        ...
        if params['use_actual_model']:
            ...
            print("Creating mesh with actual model as starting model")

# Create parametric mesh following body boundaries
            plc = mt.createParaMeshPLC(scheme, paraDepth=-ymin, boundary=0.,
                                       paraDX=0.25, paraBoundary=2)
            plc = self._create_edges(plc, bodies, points, lines)
            self.mesh = mt.createMesh(plc, quality=32.5, area=10)
            self.mesh_inv = self._associate_regions(self.mesh, bodies, points, lines)
...
# We'll handle single velocity and fixed velocity by post-processing
            ...
            self.mgr.fop.setMesh(self.mesh_inv, ignoreRegionManager=False)
            # self.mgr.fop.setMesh(self.mesh_inv, ignoreRegionManager=True)
# Get region manager
            rm = self.mgr.fop.regionManager()
...
# Apply body-specific regularization if specified
                if ib in body_regularization:
                    print(f"  Applying custom regularization for body {ib}"
                          f" (marker {marker})")
                    result = self._apply_body_regularization(
                        marker, velocity, body_regularization[ib])
                    if result is not None:
                        fixed_regions.append(result)
# Default regularization - free inversion with body velocity as starting model
                else:
                    print(f"  Region {marker}: velocity = {velocity:.1f} "
                          f"m/s (slowness = {1.0/velocity:.6f} s/m) - FREE")
                    result = self._apply_body_regularization(
                        marker, velocity, {"type": "free"})
                    if result is not None:
                        fixed_regions.append(result)

# Set starting model for each cell
            print("\nSetting cell-based starting model...")
            para = self.mgr.paraDomain
            start_model = pg.Vector(para.cellCount())
            nprop = len(bodies[0]["props"])
            sav = 1000.
            orig_cells = []
            for i, cell in enumerate(para.cells()):
                center = cell.center()
                orig_cells.append(self.mesh_inv.findCell(center))

            for i, cell in enumerate(para.cells()):
                if orig_cells[i] is None:
                    cell_velocity = sav
                else:
                    marker = orig_cells[i].marker()
    # For cells outside the defined bodies set velocity later
                    if marker == 0:
                        # continue
                        cell_velocity = sav
                    else:
    # Find the body with this marker
                        if nprop == 1:
                            cell_velocity = bodies[marker-1]["props"][0]
                        else:
                            # cell_velocity = bodies[marker-1]["props"][0]
                            body = bodies[marker-1]
                            ztop = body["top"]
                            z = self.mesh_inv.cellCenters()[i].y()
                            cell_velocity = body["props"][0] + body["props"][1]*(ztop-z)
                # start_model[i] = 1.0 / cell_velocity
                start_model[cell.id()] = 1.0 / cell_velocity
            # self.mgr.fop.setStartModel(start_model)
# # For cells outside the defined bodies set now velocity to the one of the
# #  - actually toe the average velocity of the model - to be changed
#             sav = np.mean(np.array(start_model))
#             # if len(self.bkground) > 0:
#             #     for number in self.bkground:
#             #         start_model[number[0]] = start_model[number[1]]
#             for i, cell in enumerate(self.mesh_inv.cells()):
#                 marker = cell.marker()
#                 if marker == 0:
#                     start_model[cell.id()] = sav
#             # index = np.where(np.array(start_model)==0.)[0]
#             # start_model[index] = sav

            self._cell_start_model = start_model
            for marker in range(1, len(bodies) + 1):
                index = []
                for i, c in enumerate(self.mesh_inv.cells()):
                    if c.marker() == marker:
                        index.append(i)
                para.setCellMarkers(index, marker)
...
# Create lower and upper bound vectors for all cells (in slowness)
            lower_bounds = pg.Vector(para.cellCount())
            upper_bounds = pg.Vector(para.cellCount())
...
# Allow small variation (±5%) for numerical stability
                        ...
                        if body_constraint_sources[body_idx] is None:
                            body_constraint_sources[body_idx] =\
                                f"SINGLE at {target_vel:.1f} m/s (±5%)"
                        continue
# Set correlation lengths
                    # elif reg_type == "correlation":
                    #     corr = reg_params.get('correlation', [0., 0.])
                    #     self.mgr.inv.setRegularization(
                    #         body_idx+1, correlationLengths = corr)
...
# Print summary of constraints
            for marker in range(1, len(bodies) + 1):
                body_idx = marker - 1
                cell_indices = [i for i, cell in enumerate(para.cells())
                                if orig_cells[i].marker() == marker]
                if cell_indices:
...
# Get limits for this body from first cell with this marker
                ...
                if cell_indices:
                    ...
                    max_slow = global_min_slow
                    max_slow = global_max_slow
# Set per-region limits and starting model
                region = rm.region(marker)
                region.setLowerBound(min_slow)
                region.setUpperBound(max_slow)

                print(f"  Region {marker}: limits=[{min_slow:.6f}, "
                      f"{max_slow:.6f}] s/m ([{min_vel:.1f}, "
                      f"{max_vel:.1f}] m/s)")
            region = rm.region(0)
            region.setStartModel(1./sav)
            region.setLowerBound(global_min_slow)
            region.setUpperBound(global_max_slow)

            # print("\nStarting velocities by region (all regions):")
            # for marker in range(1, 6):  # Regions 1-5
            #     velocities = []
            #     for i, cell in enumerate(para.cells()):
            #         if orig_cells[i].marker() == marker and i < len(start_model):
            #             velocities.append(1./start_model[i])
            #     if velocities:
            #         print(f"  Region {marker}: {len(velocities)} cells, "
            #               f"min={min(velocities):.1f}, "
            #               f"max={max(velocities):.1f}, "
            #               f"mean={np.mean(velocities):.1f}, "
            #               f"std={np.std(velocities):.1f} m/s")
...
# zWeight ≈ (vertical_correlation / horizontal_correlation)²
            ...
            # if horiz_corr:
            #     invert_kwargs["correlationLengths"] = [horiz_corr, vert_corr]

# Plot starting model
            self._plot_starting_model(self.inversion_folder, bodies)
            # shutil.move(os.path.join(self.inversion_folder, "starting_model.png"),
            #             os.path.join(self.inversion_folder, "starting_model1.png"))
# Run inversion
...
# Set delta phi stopping criterion directly on inversion object (as percentage)
            ...
            # for r in rm.regionIdxs():
            #     print(r, rm.region(r).startModel(), rm.region(r).limits())
...
# Don't pass limits or startModel via invert_kwargs - already set via setRegularization
            ...
            # vel_result = self.mgr.invert(startModel=start_model, **invert_kwargs)
            vel_result = self.mgr.invert(startModel=start_model, **invert_kwargs)
            # vel_result = self.mgr.invert(**invert_kwargs)

# Write files with inversion history and comparison of starting and final
# models for debug purposes
            vels = np.zeros((len(np.array(self.mgr.inv.modelHistory[0])),
                             len(self.mgr.inv.modelHistory)))  
            for i, m in enumerate(self.mgr.inv.modelHistory):
                vels[:,i] =1./ np.array(m)
            coor = np.array(para.cellCenters())
            xc = coor[:, 0]
            yc = coor[:, 1]
            # vel_result = self.mgr.fw.model
            # with open(os.path.join(self.inversion_folder,"model_history.dat"),
            #           "w") as fo:
            #     fo.write("N_cell\t marker\t marker_inv\t x  \t y   ")
            #     for i in range(vels.shape[1]):
            #         fo.write(f"\t iter_{i}")
            #     fo.write("\n")
            #     for i in range(vels.shape[0]):
            #         k = para.cells()[i].marker()
            #         kk = self.mesh_inv.cells()[i].marker()
            #         fo.write(f"{i}\t {kk}\t {k}\t {xc[i]:0.2f}\t {yc[i]:0.2f}\t ")
            #         for j in range(vels.shape[1]):
            #             fo.write(f"{vels[i,j]:0.1f}\t ")
            #         fo.write("\n")
            # with open(os.path.join(self.inversion_folder,"starting&final_model.dat"),
            #           "w") as fo:
            #     fo.write("N_cell\t marker\t marker_inv\t x  \t y   \t "
            #              "start_inv\t start_model\t final\n")
            #     for i in range(vels.shape[0]):
            #         k = para.cells()[i].marker()
            #         kk = self.mesh_inv.cells()[i].marker()
            #         fo.write(f"{i}\t {kk}\t {k}\t {xc[i]:0.2f}\t {yc[i]:0.2f}\t ")
            #         fo.write(f"{vels[i,0]:0.1f}\t {1./start_model[i]:0.1f}\t "
            #                  f"{vel_result[i]:0.1f}\n ")
...
# No post-processing clipping is done to preserve consistency with calculated travel times
            ...
            # mesh = self.mesh_inv
            # coor = np.array(mesh.cellCenters())
            # xm = coor[:, 0]
            # ym = coor[:, 1]
            # self._cell_start_model =  pg.Vector(para.cellCount())
            # for i, cell in enumerate(para.cells()):
            #     self._cell_start_model[i] = 1./vels[i, 0]
            # self._plot_starting_model(self.inversion_folder, bodies)
            # shutil.move(os.path.join(self.inversion_folder, "starting_model.png"),
            #             os.path.join(self.inversion_folder, "starting_model2.png"))
            # self.vest = np.array(self.mgr.fw.model)
            # self.vest = vels[:, -1]
            # self.mesh_inv = self.mgr.paraDomain
            # self._cell_start_model =  pg.Vector(para.cellCount())
            # with open(os.path.join(self.inversion_folder, "cells.txt"), "w") as fo:
            #     fo.write("cell#  para_id   para_mark  para_x  para_y   model_id   model_mark  model_x  model_y\n")
            #     for i, cell in enumerate(para.cells()):
            #         self._cell_start_model[i] = 1./vels[i, -1]
            #         fo.write(f"{i}\t{cell.id()}\t{cell.marker()}\t{xc[i]:0.2f}\t{yc[i]:0.2f}\t"
            #                  f"{self.mesh_inv.cells()[i].id()}\t{self.mesh_inv.cells()[i].marker()}"
            #                  f"\t{xm[i]:0.2f}\t{ym[i]:0.2f}\n")
            # self._plot_starting_model(self.inversion_folder, bodies)
            # shutil.move(os.path.join(self.inversion_folder, "starting_model.png"),
            #             os.path.join(self.inversion_folder, "final_model.png"))
            
            # print("\nStarting velocities by region (all regions):")
            # for marker in range(1, 6):  # Regions 1-5
            #     velocities = []
            #     for i, cell in enumerate(self.mesh_inv.cells()):
            #         if orig_cells[i] == marker and i < len(self.vest):
            #             velocities.append(vels[i, 0])
            #     if velocities:
            #         print(f"  Region {marker}: {len(velocities)} cells, "
            #               f"min={min(velocities):.1f}, "
            #               f"max={max(velocities):.1f}, "
                ...
                for ib in body_regularization.keys():
                    ...
                    for i, cell in enumerate(para.cells()):
                        if orig_cells[i] == marker and i < len(self.vest):
                            velocities.append(self.vest[i])
                    if velocities:
                        print(f"  Region {marker}: First 5 values: "
                              f"{[f'{v:.1f}' for v in velocities[:5]]}")
        else:
...
# Extract and store ray paths for later plotting using PyGIMLi's method
        self.ray_paths = self.get_ray_paths()
...
# Save inversion results
    ...
    def get_ray_paths(self):
        ray_paths = []  # List of ray paths, each ray is a list of (x, y) points
        try:
    ...
    def _create_edges(self, plc, bodies, points, lines):
        ...
        for il, line in enumerate(lines):
            if (not line["bottom"] and not line["topo"] and not line["left"]
                    and not line["right"]):
                pt1 = [points[line["point1"]]["x"], points[line["point1"]]["y"]]
                pt2 = [points[line["point2"]]["x"], points[line["point2"]]["y"]]
                li = mt.createPolygon([pt1, pt2], isClosed=False, marker=il+1)
                plc += li
        return plc
...
# Find nearest assigned cell
                    ...
                    cell.setMarker(0)
                    assigned_markers[cell_idx] = 0
                    self.bkground.append((cell_idx, idx))
...
# Velocity range - apply bounds by post-clipping (setRegularization doesn't
# work with cell-based)
        elif reg_type == 'range':
...
# Plot measured and calculated for this shot
            if i == 0:
                ax.plot(indices, meas_tt[start_idx:end_idx], 'b+',
                        label='Measured', markersize=8)
                ax.plot(indices, calc_tt[start_idx:end_idx], 'r-',
                        label='Calculated', linewidth=1)
            else:
    ...
    def _plot_inverted_model(self, xmin, xmax, fig=None):
...
# Draw ray paths
        ...
        ax_model.set_ylim(self.parent.ymin, self.parent.ymax)
...
# Save velocity model with cell center coordinates
        ...
        with open(model_file, 'w') as f:
            ...
            for i, cell in enumerate(self.mesh_inv.cells()):
                center = cell.center()
                f.write(f"{center[0]:.3f}  {center[1]:.3f}  {self.vest[i]:.1f}\n")
...
# Get unique shot and receiver positions to create numbering
        ...
        with open(fit_file, "w") as f:
            f.write("# Index  Shot#  Recv#  Measured[s]  Calculated[s]  "
                    "Residual[s]\n")
            for i, (m, c) in enumerate(zip(meas_tt, calc_tt)):
                shot_num = shot_to_num[shot_x[i]]
                recv_num = recv_to_num[recv_x[i]]
                f.write(f"{i:6d}  {shot_num:5d}  {recv_num:5d}  {m:.6f}  "
                        f"{c:.6f}  {m-c:.6f}\n")

        print(f"Results saved to {self.inversion_folder}")

    def _plot_starting_model(self, output_dir, bodies, axes=None):
...
# Plot mesh with velocities
        pg.show(self.mesh_inv, data=1./self._cell_start_model, ax=axes[1], 
        ...
        plt.tight_layout()
        if fig is not None:
            fig.savefig(os.path.join(output_dir, "starting_model.png"),
